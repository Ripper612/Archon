...
We use **beads (bd)** for all issue tracking instead of markdown TODOs. Beads provides dependency-aware task management that prevents AI agents from getting lost in complex workflows.
...
- `bd create "title" -t bug|feature|task -p 0-4 -d "description" --json` - Create new issues
...
3. If you discover new work while implementing, create issues with `bd create` and link with `discovered-from`
...
- Export before committing: `bd export -o .beads/issues.jsonl`
- Import after pulling: `bd import -i .beads/issues.jsonl`
